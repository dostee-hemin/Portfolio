...
class Experience {
    ...
    drawBranches() {
        ...
        if(window.scrollY < this.y || this.fractalAngle != this.targetAngle) {
            ...
            if (this.fractalAngle == this.targetAngle) {
                if(this.leaves.length == 0) {
                    for(let i=0; i<this.leafPoints.length; i++) {
                        let x = this.leafPoints[i][0] + this.x
                        let y = this.leafPoints[i][1] + this.y
                        let thickness = Math.abs(this.leafPoints[i][0])/40+3
                        this.leaves.push(new Leaf(x,y,thickness))
                    }
                }
            } else {
                ...
                for (let i=this.leaves.length-1; i>=0; i--) {
                    if (this.leaves[i].isOnBranch) this.leaves.splice(i,1);
                }
